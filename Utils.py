...
from IBLTWithOLS import IBLTWithOLS
...
#     p2_iblt.other_list_for_debug = p1_list
...
def generate_participants_iblts(universe_size: int, symmetric_difference_size: int, method: Method, set_inside_set: bool):
    """
    Generate participants lists based on the given parameters.
    ...
    tuple: A tuple containing the participants' IBLTs.
    """
    universe_array = np.arange(1, universe_size + 1, dtype=np.int64)

    # Set a unique random seed for each process based on process ID
    np.random.seed(os.getpid())

    if set_inside_set:
        p2_array = universe_array
        remove_indices = np.random.choice(universe_size, symmetric_difference_size, replace=False)
        p1_array = np.delete(universe_array, remove_indices)
    else:
        p2_size = max(symmetric_difference_size, np.random.randint(1, universe_size - symmetric_difference_size))
        p2_indices = np.random.choice(universe_size, p2_size, replace=False)
        p2_array = universe_array[p2_indices]
        
        remaining_indices = np.setdiff1d(np.arange(universe_size), p2_indices, assume_unique=True)
        p1_indices = np.concatenate([
            remaining_indices[:symmetric_difference_size - 1],
            p2_indices[:p2_size - 1]
        ])
        p1_array = universe_array[p1_indices]

    iblt_classes = {
        Method.EGH: IBLTWithEGH,
        Method.OLS: IBLTWithOLS,
        Method.EXTENDED_HAMMING: IBLTWithExtendedHamming
    }

    p1_iblt = iblt_classes[method](p1_array, universe_size, set_inside_set)
    p2_iblt = iblt_classes[method](p2_array, universe_size, set_inside_set)

    p2_iblt.other_list_for_debug = p1_array
