...
func runTrialSuccessRateVsTotalCells(trialNumber int, universeSize int, symmetricDiffSize int, rng *rand.Rand) []Result {
	...
	fmt.Printf("Trial %d with universe size %d for method Rateless IBLT, Symmetric Difference len: %d, Total Cells: %d\n", trialNumber, universeSize, symmetricDiffSize, totalCells)
	log.Printf("Trial %d with universe size %d for method Rateless IBLT, Symmetric Difference len: %d, Total Cells: %d\n", trialNumber, universeSize, symmetricDiffSize, totalCells)

	return results
}

// BenchmarkSuccessRateVsTotalCells benchmarks the reconciliation process with fixed universe size and varying symmetric difference sizes.
func BenchmarkSuccessRateVsTotalCells(b *testing.B) {
