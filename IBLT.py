...
class IBLT:
    ...
    def transmit(self) -> None:
        ...
        # for row in range(rows):
        #     # Get the indices where the row has a value of 1.
        #     mask_symbols_indices = cp.intersect1d(self.partial_mapping_matrix[row].indices, self.symbols_indices)
        #     # Get the symbols corresponding to these indices.
        #     mapped_symbols = mask_symbols_indices + 1
    ...
    def receive(self, iblt_sender_cells: List[Cell]) -> List[int]:
        ...
        self.iblt_diff_cells.extend(self.calc_iblt_diff(iblt_sender_cells,
                                                   iblt_receiver_cells))      
    ...
    def calc_iblt_diff(self, iblt_sender_cells: List[int], iblt_receiver_cells: List[int]):
        ...
        Parameters:
        - iblt_sender (List[cells]): IBLT cells of the sender (not all IBLT cells of the sender).
        - iblt_receiver (List[cells]): IBLT cells of the receiver (not all IBLT cells of the receiver).
        ...
        for cell_idx in range(len(iblt_receiver_cells)):
            ...
            iblt_diff[-1].sum =  iblt_receiver_cells[cell_idx].sum ^ iblt_sender_cells[cell_idx].sum

            if iblt_receiver_cells[cell_idx].checksum == 0:
                iblt_diff[-1].checksum = iblt_sender_cells[cell_idx].checksum
            
            elif iblt_sender_cells[cell_idx].checksum == 0:
                iblt_diff[-1].checksum = iblt_receiver_cells[cell_idx].checksum
            
            else:
                iblt_diff[-1].checksum =  bytes(a ^ b for a, b in zip(iblt_receiver_cells[cell_idx].checksum, iblt_sender_cells[cell_idx].checksum))
            
            iblt_diff[-1].counter -= iblt_receiver_cells[cell_idx].counter - iblt_sender_cells[cell_idx].counter
    ...
    def listing(self, cells: List[Cell], with_deocde_frac: bool = False) -> List[int]:
        ...
        while True:
            ...
            # np_symbol =  symbol.item()
            symbols.append(symbol)
            #symbols.append(np_symbol)

            mapped_rows = np.where(self.mapping_matrix[:, symbol-1].toarray() == 1)[0]
            
            # cp_col_array = cp.asarray(self.mapping_matrix[:, np_symbol -1].toarray())
            # mapped_rows = cp.where(cp_col_array == 1)[0].get()
